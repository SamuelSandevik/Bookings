...
#[put("/{booking_uuid}")]
pub async fn route(req: HttpRequest, path: web::Path<(Uuid, Uuid)>, body: web::Json<NewBookingDTO>) -> impl Responder {
    
    let user_profile = req.extensions().get::<UserProfile>().cloned().unwrap();


    match repo::update_status_bookings(&user_profile, &path.1, &body.0).await {
