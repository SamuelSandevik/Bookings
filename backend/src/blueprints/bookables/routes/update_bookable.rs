...
#[put("/{bookable_uuid}")]
pub async fn route(req: HttpRequest, path: web::Path<(Uuid, Uuid)>, body: web::Json<NewBookableDTO>) -> impl Responder {
    
    let user_profile = req.extensions().get::<UserProfile>().cloned().unwrap();


    match repo::update_bookable(&user_profile, &path.1, &body.0).await {
